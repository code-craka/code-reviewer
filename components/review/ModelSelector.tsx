...
const ModelSelector: React.FC<ModelSelectorProps> = ({
  ...
  return (
    <div className="mb-6">
            ...
            >
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  !isDisabled && handleCheckboxChange(model.id, checked)
                }
                disabled={isDisabled}
      ...
      {AI_MODELS_OPTIONS.find(
        (m) => m.id === AIModel.GEMINI && !isGeminiAvailable,
      ) && (
        <p className="text-xs text-yellow-500 mt-3">
          Gemini reviews might be unavailable. The server will verify API key
          access.
        </p>
      )}
    </div>
