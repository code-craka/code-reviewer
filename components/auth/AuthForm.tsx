...
export default function AuthForm({
  ...
  // Always get the intended redirect path
    ...
    startTransition(async () => {
      if (isLoginMode) {
        ...
        if (loginError) {
          ...
          // Only for password login, use router.push (not reload!)
        ...
        const { data, error: signupError } = await supabase.auth.signUp({
          ...
          password,
          options: {
            data: { user_name: username || undefined },
  ...
  const handleOAuthSignIn = async (provider: "google" | "github") => {
    ...
    startTransition(async () => {
      ...
      // No need to do anything after—Supabase will redirect to /auth/callback, which will send user to /reviewer or their `next` path
    });
  };
