...
import { Alert as ShadAlert, AlertDescription, AlertTitle } from "@/components/ui/alert"; // ShadCN UI Alert
...
const AlertComponent: React.FC<AlertProps> = ({ type, message, title, onClose, className }) => {
  const config = alertVariants[type];

  if (!message) return null;

  return (
    <ShadAlert variant={config.variant} className={cn('relative animate-fade-in', className)}>
      <span className="mr-2 flex-shrink-0">{config.icon}</span>
      <div>
        {title && <AlertTitle className={cn(config.titleClass, 'font-semibold')}>{title}</AlertTitle>}
