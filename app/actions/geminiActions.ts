...
import { GEMINI_MODEL_NAME, CODE_REVIEW_SYSTEM_INSTRUCTION } from '@/lib/constants';
...
export async function getGeminiReviewAction(code: string): Promise<ActionResponse<GeminiReviewData>> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    console.error("Gemini API key (GEMINI_API_KEY) is not defined on the server.");
  ...
  try {
    ...
    let errorMessage = `Failed to get review from Gemini. ${(error as Error).message || 'Unknown error'}`;
    if (error.message) {
        if (error.message.includes("API key not valid") || error.message.includes("API key is invalid")) {
            errorMessage = "Invalid Gemini API Key on server. Please check server configuration.";
