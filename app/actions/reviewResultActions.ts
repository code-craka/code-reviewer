 ...
 */
export async function getReviewResults(reviewId: string): Promise<ActionResponse<ReviewResult[]>> {
  ...
  try {
    ...
    const isTeamMember = review.project.team?.members.some(member => member.userId === user.id) || false;
    ...
    const results = await reviewResultService.getReviewResultsByReviewId(reviewId);
 ...
 */
export async function getProjectReviewResults(projectId: string): Promise<ActionResponse<ReviewResult[]>> {
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
  ...
  try {
    ...
    // Get the review results
    const results = await reviewResultService.getReviewResultsByProjectId(projectId);
 ...
 */
export async function createReviewResult(formData: FormData): Promise<ActionResponse<ReviewResult>> {
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
  ...
  if (!reviewId || !projectId || !fileName || !suggestion) {
    return { 
      success: false, 
      error: 'Missing required fields: reviewId, projectId, fileName, and suggestion are required' 
    };
  }
  ...
  try {
    ...
    const isTeamMember = project.team?.members.some(
      member => member.userId === user.id && ['owner', 'admin', 'member'].includes(member.role)
    ) || false;

    if (!isOwner && !isTeamMember) {
      return { success: false, error: 'You do not have permission to create review results for this project' };
    }
...
export async function updateReviewResultStatus(
  ...
  try {
    ...
    const isTeamMember = reviewResult.project.team?.members.some(
      member => member.userId === user.id && ['owner', 'admin', 'member'].includes(member.role)
    ) || false;

    if (!isOwner && !isTeamMember) {
      return { success: false, error: 'You do not have permission to update this review result' };
    }

    // Update the review result status
    const result = await reviewResultService.updateReviewResultStatus(resultId, status, user.id);
    
    revalidatePath(`/projects/${reviewResult.projectId}/reviews/${reviewResult.reviewId}`);
 ...
 */
export async function getReviewResultStats(projectId: string): Promise<ActionResponse<any>> {
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
...
export async function deleteReviewResult(resultId: string): Promise<ActionResponse<void>> {
  ...
  try {
    ...
    // Check if user has access to the project
    const isOwner = reviewResult.project.ownerId === user.id;
    const isTeamAdmin = reviewResult.project.team?.members.some(
    ...
    if (!isOwner && !isTeamAdmin) {
      return { success: false, error: 'You do not have permission to delete this review result' };
    }
