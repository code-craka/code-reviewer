...
export async function createNewProject(formData: FormData): Promise<ActionResponse<Project>> {
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
  ...
  try {
    ...
    return { success: true, data: newProject as Project, message: 'Project created successfully!' };
  } catch (error) {
...
export async function updateExistingProject(formData: FormData): Promise<ActionResponse<Project>> {
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
  ...
  try {
    ...
    return { success: true, data: updatedProject as Project, message: 'Project updated successfully!' };
  } catch (error) {
...
export async function deleteExistingProject(projectId: string): Promise<ActionResponse> {
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
...
export async function getProjectById(projectId: string): Promise<ActionResponse<Project>> {
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
