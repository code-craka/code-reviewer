     ...
     try {
      await prisma.profile.create({
        data: {
          id: user.id,
          email: user.email || '',
          username: user.user_metadata?.user_name || (user.email ? user.email.split('@')[0] : 'user'),
          profilePictureUrl: user.user_metadata?.avatar_url
        }
      });
    } catch (error) { 
        console.warn("Could not create profile on the fly for user:", user.id, error);
    }
  }
...
export default async function ReviewerPage() {
  ...
  if (session?.user) {
    const profileResult = await prisma.profile.findUnique({ where: { id: session.user.id } });
    ...
    <div className="flex flex-col min-h-screen">
      ...
      >
         <header className="mb-8 text-center">
            <h1 className="text-4xl sm:text-5xl font-bold text-primary animate-fade-in">AI Code Reviewer</h1>
            <p className="text-muted-foreground mt-2 text-sm sm:text-base animate-fade-in" style={{animationDelay: '0.2s'}}>
        ...
        <Suspense fallback={
          <div className="flex flex-col justify-center items-center p-10 min-h-[400px]">
            ...
            <p className="ml-4 text-lg text-muted-foreground mt-4">Loading Reviewer...</p>
          </div>
        }>
          <ReviewerPageContent />
        </Suspense>
