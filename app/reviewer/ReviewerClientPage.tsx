...
import {
  ...
  Review,
} from "@/types/index"; // Added Review
import { AI_MODELS_OPTIONS } from "@/lib/constants";
import {
  getGeminiReviewAction,
  GeminiReviewData,
...
export default function ReviewerClientPage({
  initialProjects,
}: ReviewerClientPageProps) {
  ...
  const [currentCodeInEditor, setCurrentCodeInEditor] = useState<string>(
    "// Welcome! Select a project or start coding in scratchpad mode.",
  );
  const [newProjectNameFromReviewer, setNewProjectNameFromReviewer] =
    useState("");
  ...
  useEffect(() => {
    ...
    if (projectIdFromQuery) {
      if (currentProject?.id !== projectIdFromQuery) {
        // Only load if different project
          ...
          if (response.success && response.data) {
            ...
            setGlobalError(
              (response.error as string) ||
                `Could not load project ${projectIdFromQuery}.`,
            );
      ...
      setCurrentCodeInEditor(
        "// Start coding in scratchpad mode or select a project.",
      );
    }
  ...
  const handleGetReviewSubmit = useCallback(async () => {
    ...
    startTransition(async () => {
      for (const modelId of selectedModels) {
        const modelOption = AI_MODELS_OPTIONS.find(
          (opt) => opt.id === modelId,
        )!;
        try {
          let reviewResponseData: GeminiReviewData | MockReviewResponse | null =
            null;
          if (modelId === AIModel.GEMINI) {
            const geminiResult =
              await getGeminiReviewAction(currentCodeInEditor);
            if (!geminiResult.success)
              throw new Error(
                (geminiResult.error as string) || "Gemini review failed",
              );
            reviewResponseData = geminiResult.data!;
          } else {
            reviewResponseData = await getMockReview(
              modelOption.name,
              currentCodeInEditor,
            );
          }
          setReviews((prev) => ({
            ...
            [modelId]: {
              ...prev[modelId]!,
              feedback: reviewResponseData!.feedback,
              tokensUsed: reviewResponseData!.tokensUsed,
              isLoading: false,
              error: null,
            },
          }));
          ...
          setReviews((prev) => ({
            ...
            [modelId]: {
              ...
              error:
                ...
                `Failed to get review from ${modelOption.name}`,
              isLoading: false,
            },
          }));
        }
  ...
  const handleCreateNewProjectFromReviewer = async (e: FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (result.success && result.data) {
        setGlobalInfo(
          `Project "${result.data.name}" created and current code saved.`,
        );
        setCurrentProject(result.data);
        setProjects((prev) => [result.data!, ...prev]);
        setIsNewProjectModalOpen(false);
        setNewProjectNameFromReviewer("");
        router.push(`${pathname}?projectId=${result.data.id}`, {
          scroll: false,
        });
      } else {
        setGlobalError(
          (result.error as string) || "Failed to create new project.",
        );
      }
    });
  };
  ...
  const refreshProjects = useCallback(() => {
    startTransition(async () => {
      ...
      if (result.success && result.data) {
        ...
        if (currentProject) {
          const updatedCurrent = result.data.find(
            (p) => p.id === currentProject.id,
          );
          if (!updatedCurrent) {
            // Current project might have been deleted elsewhere
            handleSwitchToScratchpad();
          } else if (
            (updatedCurrent.code || "") !== (currentProject.code || "") &&
            (updatedCurrent.code || "") !== currentCodeInEditor
          ) {
            ...
            setGlobalInfo(
              `Project "${updatedCurrent.name}" reloaded with latest changes.`,
            );
          }
        }
  ...
  const codeHasUnsavedChanges =
    currentProject && currentCodeInEditor !== (currentProject.code || "");
  const isGeminiAvailable = true; // Server handles API key validation now
  const isAnyReviewLoading = Object.values(reviews)
    .filter(Boolean)
    .some((r: Review) => r.isLoading);
      ...
      <div className="bg-card shadow-2xl rounded-lg p-6 sm:p-8 mb-8">
        ...
        {!currentProject && (
          <div className="mb-4 p-3 bg-muted border border-border rounded-md text-sm text-muted-foreground">
            Currently in scratchpad mode. Changes are not saved unless you
            create a new project.
          </div>
        )}

        <div className="mb-4">
          <Label htmlFor="project-selector" className="mb-1">
            Load Project:
          </Label>
            ...
            >
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    The current code in the editor will be saved as the initial
                    code for this new project.
                  </DialogDescription>
                </DialogHeader>
                <form
                  onSubmit={handleCreateNewProjectFromReviewer}
                  className="space-y-4"
                >
                  <div>
                    <Label htmlFor="newProjectNameFromReviewer">
                      Project Name
                    </Label>
                    <Input
                      ...
                      onChange={(e) =>
                        setNewProjectNameFromReviewer(e.target.value)
                      }
                      required
