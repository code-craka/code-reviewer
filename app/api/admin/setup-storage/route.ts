...
export async function POST() {
  ...
  try {
    ...
    const bucketExists = buckets.some(
      (bucket) => bucket.name === AVATARS_BUCKET,
    );
    ...
    for (const policy of AVATAR_POLICIES) {
      console.log(
        `Creating policy: ${policy.name} for operation ${policy.operation}`,
      );
      try {
        ...
        if (policyError) {
          console.warn(
            `Warning: Error creating policy ${policy.name}:`,
            policyError,
          );
          policyResults.push({
            name: policy.name,
    ...
    const { error: s3Error } = await supabaseAdmin.storage.updateBucket(
      AVATARS_BUCKET,
      {
        public: false,
        allowedMimeTypes: [
