      ...
      () => {
        ...
        if (code.trim().length === 0) {
          ...
          if (code.includes("TODO")) {
            reviewText +=
              "- Found 'TODO' comments. Ensure these are addressed before production.\n";
          }
          if (
            code.toLowerCase().includes("password") &&
            !code.toLowerCase().includes("hashedpassword")
          ) {
            reviewText +=
              "- Warning: Detected plain text 'password'. Ensure sensitive data is handled securely.\n";
          }
        }
